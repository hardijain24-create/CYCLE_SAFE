...
def train_cyclesafe_V4(
    ...
    # --- Per-variability-group coverage on FINAL TEST using Mondrian conformal radii ---
    per_group_coverage = {}
    if conformal_radii and cv_tertile_cutoffs and "cv_5" in final_test.columns:
        cut_low, cut_high = cv_tertile_cutoffs
        for g_label, g_cond in [
            ("low", final_test["cv_5"] < cut_low),
            ("medium", (final_test["cv_5"] >= cut_low) & (final_test["cv_5"] < cut_high)),
            ("high", final_test["cv_5"] >= cut_high),
        ]:
            g_mask = g_cond.to_numpy()
            g_actual = final_test["target"].to_numpy()[g_mask]
            g_pred = final_test_pred[g_mask]
            n_g = len(g_actual)
            if n_g == 0:
                continue
            r80 = conformal_radii[g_label]["radius80"]
            r90 = conformal_radii[g_label]["radius90"]
            g_errors = np.abs(g_actual - g_pred)
            cov80 = float(np.mean(g_errors <= r80))
            cov90 = float(np.mean(g_errors <= r90))
            width80 = 2 * r80
            width90 = 2 * r90
            per_group_coverage[g_label] = {
                "n": int(n_g),
                "radius80": float(r80),
                "radius90": float(r90),
                "coverage_80": round(cov80, 4),
                "coverage_90": round(cov90, 4),
                "mean_width_80": round(width80, 2),
                "mean_width_90": round(width90, 2),
            }
        
        print("\nPER-GROUP FINAL TEST COVERAGE (Mondrian conformal radii):")
        for g, m in per_group_coverage.items():
            print(f"  {g}: n={m['n']} | 80%: {m['coverage_80']*100:.1f}% (w={m['mean_width_80']:.1f}d) | 90%: {m['coverage_90']*100:.1f}% (w={m['mean_width_90']:.1f}d)")
...
if __name__ == "__main__":
    ...
    generate_doctor_report_pdf(regular_report, 'reports/doctor_report_regular.pdf')
    generate_doctor_report_pdf(peri_report, 'reports/doctor_report_perimenopause.pdf')
    generate_doctor_report_pdf(meno_report, 'reports/doctor_report_menopause.pdf')
